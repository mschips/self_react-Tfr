...
if (LEGENDUSE!='Apoptosis') {
    ...
    MAIN_FIG<-ggarrange(gcbc_num_plot,
        ...
        if (LEGENDUSE=='SemiGate') {figtitle<-'Figure6.png'; supptitle<-'FigS6.png'}
        else if (LEGENDUSE=='SemiGate.38') {figtitle<-'Figure7.png'; supptitle<-'FigS7.png'}
#     ggsave(paste0('P_',RESULTSET,select_pself,'.png'),h=H_PLOT,w=W_PLOT, dpi=300)
    ...
    MAIN_FIG<-ggarrange(gcbc_num_plot,
        ...
        outaff_box_plot+theme(axis.title.x=element_blank()),
        outself_box_plot,
        labels='AUTO', common.legend=T,font.label = list(size = 20), ncol=NCOL_PLOT)

    ggsave('Figure4.png',h=H_PLOT,w=W_PLOT, dpi=300)
    ###
    SUPP_FIG<-ggarrange(
        perc_self_gcbc_plot+theme(axis.title.x=element_blank()), 
        perc_self_out_plot+theme(axis.title.x=element_blank()),
    ...
    ggsave(paste0('P_SUPP_',RESULTSET,select_pself,'.png'),h=H_PLOT,w=W_PLOT, dpi=300)
